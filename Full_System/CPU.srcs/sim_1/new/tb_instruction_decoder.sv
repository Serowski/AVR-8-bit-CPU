...
module tb_instruction_decoder;
    ...
    task automatic check(
        ...
        input logic [15:0] got_sram,
        input logic [15:0] exp_sram
    );
        if (got_itype === exp_itype && got_rd === exp_rd && got_rr === exp_rr && got_imm === exp_imm &&
            got_aluop === exp_aluop && got_pc === exp_pc && got_sram === exp_sram) begin
            ...
            if (got_sram !== exp_sram)
                $display("         sram  : got=0x%04h  exp=0x%04h", got_sram, exp_sram);
