...
// End-to-end test: UART upload followed by CPU execution
// 1) Reset = 0: UART uploads program
// 2) Reset = 1: CPU runs the program
// 3) Register values are checked
...
module tb_cpu_all;
    ...
    // receive one byte from the TX echo
    task automatic receive_byte(output [7:0] val);
        integer i;
        begin
            @(negedge o_Tx_data);      // wait for start bit
    ...
    initial begin
        ...
        // begin with button released (i_rst_n = 1)
        // CPU runs, UART held in reset
        i_rst_n = 1;
        #(CLK_PERIOD * 10);

        // ============================================
        // PHASE 1: Upload program via UART
        //   i_rst_n = 0 -> CPU halted, UART active
        // ============================================
        // press button (i_rst_n = 0) to activate UART
        i_rst_n = 0;
        #(CLK_PERIOD * 20);

        $display("--- PHASE 1: Upload program via UART ---");
        $display("  Program: LDI R16,5 | LDI R17,3 | ADD R16,R17 | RJMP -1");
        $display("");

        // Instruction 0: LDI R16, 5  (0xE005)
        send_and_verify(8'h05, "LDI_R16_lo");
        send_and_verify(8'hE0, "LDI_R16_hi");

        // Instruction 1: LDI R17, 3  (0xE013)
        send_and_verify(8'h13, "LDI_R17_lo");
        send_and_verify(8'hE0, "LDI_R17_hi");

        // Instruction 2: ADD R16, R17  (0x0F01)
        send_and_verify(8'h01, "ADD_lo    ");
        send_and_verify(8'h0F, "ADD_hi    ");

        // Instruction 3: RJMP -1  (0xCFFF) -- infinite loop
        //send_and_verify(8'hFF, "RJMP_lo   ");
        //send_and_verify(8'hCF, "RJMP_hi   ");

        $display("");
        $display("  Upload done! 4 instructions (8 bytes)");
        ...
        // ============================================
        // PHASE 2: Start CPU
        //   i_rst_n = 1 -> CPU runs, UART in reset
        // ============================================
        $display("");
        $display("--- PHASE 2: CPU execution ---");

        #(CLK_PERIOD * 5);
        i_rst_n = 1;
        $display("  i_rst_n = 1 (CPU START) at t=%0t", $time);

        // let the CPU execute (4 instructions x 4 FSM cycles = ~16 clocks; give it 50)
        ...
        // ============================================
        $display("");
        $display("--- PHASE 3: Register verification ---");

        $display("  R16 = %0d (expected: 8)",
            dut.u_execution_unit.eu_reg_file.regs[16]);
        $display("  R17 = %0d (expected: 3)",
    ...
    // per-cycle trace of key signals
